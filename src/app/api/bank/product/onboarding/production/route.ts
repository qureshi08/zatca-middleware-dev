...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${integration.middlewareBaseUrl}/api/v1/zatca/onboarding/production`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { error: e?.message || 'Failed to reach middleware production endpoint' },
      { status: 502 }
    );
  }
}
